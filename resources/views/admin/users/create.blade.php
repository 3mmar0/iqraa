...
@section('content')
    <form method="POST" action="{{ route('admin.users.store') }}" class="mx-auto max-w-3xl space-y-6">
        ...
        <section class="rounded-2xl border border-[var(--color-line)] bg-white p-5 sm:p-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="university" name="university" value="{{ old('university') }}" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-[var(--color-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20">
                </div>
                <div>
            ...
            <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($roles as $role)
                    <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 px-3 py-3 text-sm hover:border-[var(--color-primary)] hover:bg-[var(--color-primary-light)]/40">
                        <input type="checkbox" name="roles[]" value="{{ $role->id }}" @checked(collect(old('roles', []))->contains($role->id) || $role->slug === 'student') class="rounded border-slate-300 text-[var(--color-primary)] focus:ring-[var(--color-primary)]">
                        <span>
        ...
        <div class="flex flex-wrap gap-3">
            <button type="submit" class="rounded-xl bg-[var(--color-primary)] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[var(--color-primary-hover)]">حفظ المستخدم</button>
