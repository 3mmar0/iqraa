...
<div>
    ...
    <input id="title" name="title" value="{{ old('title', $a?->title) }}" required class="admin-input">
</div>
<div>
    <label class="admin-label" for="description">الوصف</label>
    <textarea id="description" name="description" rows="4" class="admin-input">{{ old('description', $a?->description) }}</textarea>
</div>
<div class="grid gap-4 sm:grid-cols-2">
    ...
    <div>
        <label class="admin-label" for="lesson_id">الدرس (اختياري)</label>
        <select id="lesson_id" name="lesson_id" class="admin-input">
            <option value="">—</option>
    ...
    <div>
        <label class="admin-label" for="due_at">موعد التسليم</label>
        <input id="due_at" type="datetime-local" name="due_at" value="{{ old('due_at', $a?->due_at?->format('Y-m-d\TH:i')) }}" required class="admin-input">
    </div>
    <div>
