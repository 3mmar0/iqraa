<form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-5">
    ...
    <label class="flex items-center gap-3 text-sm text-slate-700">
        ...
        <input type="checkbox" name="backup_enabled" value="1" @checked(old('backup_enabled', $settings['backup.enabled'] ?? true)) class="rounded border-slate-300 text-[var(--color-primary)]">
