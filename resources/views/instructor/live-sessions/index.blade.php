...
@section('content')
    <h1 class="mb-6 text-2xl font-bold text-teal-900">الجلسات المباشرة</h1>
    ...
    @if (\Illuminate\Support\Facades\Route::has('instructor.live-sessions.store'))
        <form method="POST" action="{{ route('instructor.live-sessions.store') }}" class="mb-8 max-w-xl space-y-3 rounded-xl border border-slate-200 bg-white p-4">
            ...
            <button type="submit" class="rounded bg-teal-700 px-4 py-2 text-white">جدولة</button>
        </form>
    @endif
        ...
        <ul class="space-y-3">
            @foreach ($sessions as $session)
                <li class="rounded-xl border border-slate-200 bg-white p-4">
                    ...
                    @if ($session->join_url)
                        <a href="{{ $session->join_url }}" class="text-sm text-teal-700 hover:underline" target="_blank" rel="noopener">رابط الانضمام</a>
                    @endif
                </li>
            @endforeach
        </ul>
