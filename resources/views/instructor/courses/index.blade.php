...
@section('content')
    <div class="mb-6 flex items-center justify-between gap-4">
        <h1 class="text-2xl font-bold text-[var(--color-ink)]">مقرراتي</h1>
        @if (\Illuminate\Support\Facades\Route::has('instructor.courses.create'))
            <a href="{{ route('instructor.courses.create') }}" class="rounded bg-[var(--color-primary)] px-4 py-2 text-sm text-white">مقرر جديد</a>
        @endif
    </div>
                ...
                <li class="rounded-xl border border-slate-200 bg-white p-4">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        @if (\Illuminate\Support\Facades\Route::has('instructor.courses.show'))
                            <a href="{{ route('instructor.courses.show', $course) }}" class="text-sm text-[var(--color-primary)] hover:underline">عرض</a>
                        @endif
                    </div>
                </li>
            @endforeach
