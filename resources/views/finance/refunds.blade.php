...
@section('content')
    <h1 class="mb-6 text-2xl font-bold text-[var(--color-ink)]">الاستردادات</h1>
    @if (\Illuminate\Support\Facades\Route::has('finance.refunds.store'))
        <form method="POST" action="{{ route('finance.refunds.store') }}" class="mb-8 max-w-lg space-y-3 rounded-xl border border-slate-200 bg-white p-4">
            ...
            <button type="submit" class="rounded bg-[var(--color-primary)] px-4 py-2 text-white">تسجيل استرداد</button>
        </form>
    @endif
    @if ($refunds->isEmpty())
