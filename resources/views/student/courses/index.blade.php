...
@section('content')
    ...
    @if ($courses->isEmpty())
        <div class="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-600">
            ...
            <a href="{{ route('student.course-requests.index') }}" class="mt-2 block text-[var(--color-primary)] hover:underline">اطلب مقرراً</a>
        </div>
    @else
            ...
            @foreach ($courses as $course)
                <article class="rounded-xl border border-slate-200 bg-white p-4">
                    ...
                    <div class="mt-2 h-2 overflow-hidden rounded bg-slate-100">
                        <div class="h-full bg-[var(--color-primary)]" style="width: {{ $course->progress_percent }}%"></div>
                    </div>
                    <a href="{{ route('student.courses.show', $course) }}" class="mt-3 inline-block text-sm text-[var(--color-primary)] hover:underline">دخول</a>
                </article>
            @endforeach
        </div>
    @endif
