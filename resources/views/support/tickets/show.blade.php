...
@section('content')
    <h1 class="mb-2 text-2xl font-bold text-[var(--color-ink)]">{{ $ticket->subject }}</h1>
    ...
    @if ($ticket->status !== 'closed')
        @if (\Illuminate\Support\Facades\Route::has('support.tickets.reply'))
            <form method="POST" action="{{ route('support.tickets.reply', $ticket) }}" class="mb-4 max-w-xl space-y-3">
                ...
                <button type="submit" class="rounded bg-[var(--color-primary)] px-4 py-2 text-white">رد</button>
            </form>
        @endif
