...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <a href="{{ route('public.courses.index') }}" class="text-sm font-medium text-[var(--color-primary-hover)] hover:underline">← العودة للكتالوج</a>
        ...
        @auth
            @if (\Illuminate\Support\Facades\Route::has('student.course-requests.index'))
                <a href="{{ route('student.course-requests.index', ['course_id' => $course->id]) }}"
                   class="mt-6 inline-block rounded-xl bg-[var(--color-primary)] px-5 py-2.5 font-medium text-white hover:bg-[var(--color-primary-hover)]">طلب الانضمام</a>
            @endif
        @else
            <a href="{{ route('login') }}" class="mt-6 inline-block rounded-xl bg-[var(--color-primary)] px-5 py-2.5 font-medium text-white hover:bg-[var(--color-primary-hover)]">طلب الانضمام</a>
