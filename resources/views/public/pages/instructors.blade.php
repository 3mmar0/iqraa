            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($instructors as $instructor)
                    <article class="rounded-2xl border border-[var(--color-line)] bg-white p-6">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-teal-800 text-lg font-bold text-teal-100">
                        ...
                        <p class="mt-4 text-sm text-teal-800">{{ $instructor->published_courses_count }} مقرر منشور</p>
                        @if ($instructor->published_courses_count > 0)
                            <a href="{{ route('public.courses.index') }}" class="mt-3 inline-block text-sm font-medium text-teal-800 hover:underline">عرض المقررات</a>
                        @endif
                    </article>
                @endforeach
            </div>
