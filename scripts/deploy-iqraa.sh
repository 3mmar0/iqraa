...
APP=/var/www/clients/iqraa.ammarelgndy.cloud
cd "$APP"
...
sudo supervisorctl stop 'iqraa-queue:*' || true
sudo supervisorctl start 'iqraa-queue:*' || echo "WARN: supervisorctl start failed (continuing)"
